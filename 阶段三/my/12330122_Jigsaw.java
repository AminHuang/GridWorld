...
public class Jigsaw {
	...
	public boolean BFSearch() throws IOException {
		...
		// (2-3)寻找所有与currentJNode邻接且未曾被访问的节点，插入openList中
	...
	private void estimateValue(JigsawNode jNode) {
		// 后续节点不正确的数码个数
		int s = 0; 
		// 所有放错位的节点与其正确位置的曼哈顿距离
		int p = 0;
		// 所有放错位的数码的个数
		int h = 0;
		// 所有放错位的节点与其正确位置的欧几理德距离（直线距离）
		int f = 0;
		
		int dimension = JigsawNode.getDimension();
		
		for (int index = 1; index < dimension * dimension; index++) {
			int t = endJNode.getNodesState()[index+1] - endJNode.getNodesState()[index];
			// 横向后续节点不正确
			if (jNode.getNodesState()[index] + t != endJNode.getNodesState()[index + 1]) {
				s++;
			}
		}
		for (int index = 1; index < dimension * (dimension - 1); index++) {
			int t = endJNode.getNodesState()[index+dimension] - endJNode.getNodesState()[index];
			// 纵向后续节点不正确
			if (jNode.getNodesState()[index] + t != endJNode
		...
		for (int i = 1; i <= dimension * dimension; i++) {
			// 计算欧几理德距离
			if (jNode.getNodesState()[i] != endJNode.getNodesState()[i] && jNode.getNodesState()[i] != 0) {
				int realRow = (jNode.getNodesState()[i] - 1) / dimension;
				int realCol = (jNode.getNodesState()[i] - 1) % dimension;
				int curRow = (endJNode.getNodesState()[i] - 1) / dimension;
				int curCol = (endJNode.getNodesState()[i] - 1) % dimension;
				
				// 曼哈顿距离
				p += Math.abs(realRow - curRow) + Math.abs(realCol - curCol);
				// 计算欧几理德距离
				f += (int)Math.sqrt(Math.pow((realRow - curRow),2) + Math.pow((realCol - curCol),2));
			}
			// 所有放错位的数码的个数
		...
		jNode.setEstimatedValue(s + p*10 + f*5+ h);
	}
